...
/// Probed state of a [`DirectoryResource`] atom.
///
/// `Sourced` (and the two `NotSourced*` variants) mirror
/// [`FileState`](super::file::FileState): in
/// [`ApplyMode::Local`](lusid_ctx::ApplyMode::Local) the desired shape is a
/// symlink at `path` targeting `source`; in
/// [`ApplyMode::Guest`](lusid_ctx::ApplyMode::Guest) the desired shape is a
/// regular directory at `path` (a recursive copy of `source` lives there
/// after the [`DirectoryOperation::CopyTree`] operation runs).
#[derive(Debug, Clone)]
pub enum DirectoryState {
    Sourced,
    NotSourcedAsCopyTree,
    NotSourcedAsSymlink,
    Present,
    Absent,
    ModeCorrect,
...
impl Display for DirectoryState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let text = match self {
            Sourced => "Sourced",
            NotSourcedAsCopyTree => "NotSourcedAsCopyTree",
...
pub enum DirectoryChange {
    ...
    /// Materialise `path` as a symlink to `source` — used for sourced
    /// directories in [`ApplyMode::Local`](lusid_ctx::ApplyMode::Local).
    CreateSymlink {
        source: FilePath,
        path: FilePath,
    },
    /// Recursively copy `source` to `path` — used for sourced directories in
    /// [`ApplyMode::Guest`](lusid_ctx::ApplyMode::Guest).
    CopyTree {
        source: FilePath,
        path: FilePath,
    },
...
impl ResourceType for Directory {
    ...
    fn resources(params: Self::Params) -> Vec<CausalityTree<Self::Resource>> {
        // mode/user/group sub-atoms are emitted the same way for both
        // `Sourced` and `Present` — the user-facing param surface is the same
        // ("set this directory's permissions"), and at apply time chmod/chown
        // follow symlinks on Linux, so a sourced-symlink in local mode ends
        // up applying these to the source directory in the operator's
        // config repo. That's intentional and accepted.
        fn permission_atoms(
            path: &FilePath,
            mode: Option<FileMode>,
            user: Option<FileUser>,
    ...
    fn change(resource: &Self::Resource, state: &Self::State) -> Option<Self::Change> {
        match (resource, state) {
            (DirectoryResource::Sourced { source, path }, DirectoryState::NotSourcedAsCopyTree) => {
            ...
            (DirectoryResource::Sourced { source, path }, DirectoryState::NotSourcedAsSymlink) => {
...
/// Probe the apply-mode-appropriate state of a sourced directory.
///
/// Local mode — the desired shape is a symlink at `path` whose target equals
/// `source`. Probe with `lstat` + `readlink`; a regular directory at `path`
/// reports `NotSourcedAsSymlink`.
///
/// Guest mode — the desired shape is a regular directory at `path` (a tree
/// copied from `source`). The state probe is intentionally weak: existence
/// of a directory at `path` reports `Sourced`. Content drift in `source`
/// after a successful first apply is not detected; the operator can re-trigger
/// by declaring `state: "absent"` and re-applying. (Note(cc): a content-aware
/// recursive diff is a follow-up — see Salt's `file.recurse`.)
async fn probe_sourced_state(
    ...
    match mode {
        // Note(cc): symlink-target comparison is lexical — same rationale
        // as the file-resource probe. See the matching comment in
        // `resource/src/resources/file.rs::probe_sourced_state`.
        ApplyMode::Local => match fs::probe_symlink(path.as_path()).await? {
            fs::SymlinkTarget::Symlink(target) if target == source.as_path() => {
                Ok(DirectoryState::Sourced)
            }
            // Wrong-target symlink, real directory, or missing path — all
            // route to "(re)create the symlink".
            _ => Ok(DirectoryState::NotSourcedAsSymlink),
        },
        ApplyMode::Guest => {
            if !fs::path_exists(path.as_path()).await? {
                return Ok(DirectoryState::NotSourcedAsCopyTree);
            }
            // Weak: directory exists ⇒ Sourced. Content drift is not detected
            // (see fn-level docs).
            Ok(DirectoryState::Sourced)
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn local_correct_dir_symlink_reports_sourced() {
    ...
    #[tokio::test]
    async fn local_real_directory_reports_not_sourced_as_symlink() {
        let dir = tempdir().unwrap();
        let source = dir.path().join("src");
        tokio::fs::create_dir(&source).await.unwrap();
        ...
        let state = probe_sourced_state(ApplyMode::Local, &file_path(&source), &file_path(&target))
            .await
            .unwrap();
        assert!(matches!(state, DirectoryState::NotSourcedAsSymlink));
    }
    ...
    #[tokio::test]
    async fn guest_existing_dir_reports_sourced_weakly() {
        // Guest mode is intentionally weak: any directory at `path` is taken
        // to mean "already sourced", regardless of content. This test pins
        // that behavior so a future refactor can't silently strengthen it.
        let dir = tempdir().unwrap();
        let source = dir.path().join("src");
        tokio::fs::create_dir(&source).await.unwrap();
        let target = dir.path().join("dest");
        tokio::fs::create_dir(&target).await.unwrap();
        // Add a file only to the source — guest mode should still say `Sourced`.
    ...
    #[tokio::test]
    async fn guest_missing_path_reports_not_sourced_as_copy_tree() {
        let dir = tempdir().unwrap();
        let source = dir.path().join("src");
        tokio::fs::create_dir(&source).await.unwrap();
        let target = dir.path().join("dest");
        ...
        let state = probe_sourced_state(ApplyMode::Guest, &file_path(&source), &file_path(&target))
            .await
            .unwrap();
        assert!(matches!(state, DirectoryState::NotSourcedAsCopyTree));
    }

    /// Pin the change-emission table for the new sourced variant: copy-tree
    /// vs symlink. These two flow into different `DirectoryOperation` arms and
    /// would silently swap if the match arms were edited.
    #[test]
    fn change_for_not_sourced_as_copy_tree_emits_copy_tree() {
        let resource = DirectoryResource::Sourced {
            source: FilePath::new("/host/src"),
            path: FilePath::new("/target/dest"),
        };
        let change = Directory::change(&resource, &DirectoryState::NotSourcedAsCopyTree)
            .expect("some change");
        match change {
            DirectoryChange::CopyTree { source, path } => {
    ...
    #[test]
    fn change_for_not_sourced_as_symlink_emits_create_symlink() {
        let resource = DirectoryResource::Sourced {
            source: FilePath::new("/host/src"),
            path: FilePath::new("/target/dest"),
        };
        let change = Directory::change(&resource, &DirectoryState::NotSourcedAsSymlink)
            .expect("some change");
        match change {
            DirectoryChange::CreateSymlink { source, path } => {
                assert_eq!(source.as_path(), std::path::Path::new("/host/src"));
    ...
    // See the matching note in `file.rs` tests — no `(Sourced, Sourced)
    // -> None` test, the match arm + catch-all panic structurally enforces it.
}
