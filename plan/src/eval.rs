...
pub(crate) fn evaluate(
    ...
    let args = match params_value {
        ...
        Some(params_value) => {
            ...
            // Tagged: a nested module receiving a forwarded `HostPath` /
            // `TargetPath` should be able to discriminate it from a raw user
            // string of the wrong kind. See `params/src/lib.rs`.
