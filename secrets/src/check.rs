...
//! `recipients.toml`. CI-friendly: exits non-zero on any finding.
//!
//! Findings collected:
//!
//! - **orphan** — a `*.age` file exists with no matching `[files]` entry.
//!   These won't decrypt via the normal apply path (plans only see files
//!   listed in `recipients.toml`) so they're either stale or mis-named.
...
//!   current `recipients.toml` (needs a `rekey`).
//!
//! ## Drift precision
//!
...
use crate::recipients::{Key, Recipients, ResolveError, ResolvedRecipient};
...
pub async fn check(secrets_dir: &Path, recipients: &Recipients) -> Result<CheckReport, CheckError> {
    ...
    // Orphans: .age files with no recipients.toml entry.
    ...
    // Missing + resolve + drift: walk recipients.toml's [files].
...
pub(crate) fn compare_stanzas(
    ...
    let expected_ssh_tags: Vec<(String, String)> = resolved
        .iter()
        .filter_map(|r| r.key.ssh_stanza_tag().map(|tag| (r.alias.clone(), tag)))
        .collect();
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            DriftReason::UnexpectedSshTag { tag } => {
                write!(f, "unexpected SSH tag {tag} not listed in recipients.toml")
            }
